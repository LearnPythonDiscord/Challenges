...
class TestRunner:
    """Test runner for challenge test suite."""
    ...
    def write_footer(self, result: ChallengeTestResult, duration: float) -> None:
    ...
    def run(self, test: unittest.TestSuite) -> None:
        ...
        result = ChallengeTestResult(self.stream)
        self.write_header()

        # Record the start time
        start = timeit.default_timer()

        # Pass the ChallengeTestResult instance to the test suite to run the tests
        test(result)
...
class ChallengeTestResult(unittest.TestResult):
...
def main() -> None:
    ...
    test_suite = test_loader.loadTestsFromName("test_challenges")
    runner = TestRunner()
